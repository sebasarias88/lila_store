  ...
  return (
    <footer className="catalog-footer mt-16 border-t border-[var(--border)] bg-[var(--bg-footer)]">
      <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-10 py-12 sm:grid-cols-2 lg:grid-cols-3 lg:gap-12 lg:py-14">
          <div>
            ...
            >
              ...
              <p className="brand-wordmark text-[18px] text-[var(--accent-deep)]">
                {nombreNegocio}
              </p>
            </Link>
            ...
            <nav className="flex flex-col gap-2.5">
              {navLinks.map(({ href, label }) => (
                <Link
                  key={href}
                  href={href}
                  className="text-[14px] font-semibold text-[var(--text-secondary)] transition-colors hover:text-[var(--accent-deep)]"
                >
                  {label}
                </Link>
              ))}
        ...
        <div className="flex flex-col items-center justify-between gap-3 border-t border-[var(--border)] py-6 sm:flex-row">
          <p className="text-center text-[12px] font-medium text-[var(--text-muted)] sm:text-left">
